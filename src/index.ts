  ...
  addItem(config: menuItemConfig, isHeading = false) {
    ...
    if (this.element.classList.contains("active"))
      this.DDMinstance.menuCon.style.height =
        (this.element.querySelector<HTMLElement>(".scene")?.offsetHeight ??
          100) + "px";
  }
...
class dropDownMenu {
  ...
  constructor(scenes: Array<menuSceneConfig>, menuCon: HTMLElement) {
    ...
    for (const scene of scenes)
      if (!this.scenes[scene.id].element) this.makeScene(scene);
      ...
      for (const sceneID in this.scenes) {
        if (sceneID === id) {
          ...
          this.menuCon.style.height =
            this.scenes[sceneID].element.querySelector<HTMLElement>(".scene")
              .offsetHeight + "px";
  ...
  deleteSceneFromHistory(val: string) {
    for (let i = this.history.length - 1; i >= 0; i--)
      if (this.history[i] == val) this.history.splice(i, 1);
  }
